...
public:
  ...
  glm::mat4 GetLightSpaceMatrix() {
    ...
    glm::mat4 light_view =
        glm::lookAt(light_position, glm::vec3(0.0f, 0.0f, 0.0f),
...
struct Camera {
  ...
  Camera()
      : camera_position(glm::vec3(0.0, 0.0, 30.0f)),
        camera_front(glm::vec3(0, 0, -1)), camera_up(glm::vec3(0, 1, 0)),
...
class Renderer {
public:
  Renderer() : NO_LIGHTING(false), FLAT_COLOR_SHADING(false) { Init(); }
  Camera camera;
  ...
  void DrawObject(float size, glm::vec3 position, unsigned model_id, int index,
